...
(ggplot(week7_tbl, aes(x = timeStart, y = q1)) +
  ...
  ggsave(filename = "../figs/fig1.png", dpi = 600, width = 6, height = 4, units = "in") 
...
(ggplot(week7_tbl, aes(x = q1, y = q2, color = gender)) +
  ...
  coord_fixed()) %>%
  ggsave(filename = "../figs/fig2.png", dpi = 600, width = 6, height = 4, units = "in")
(ggplot(week7_tbl, aes(x = q1, y = q2)) +
  geom_point(position = "jitter") +
  facet_grid(cols = vars(gender))  + 
  coord_fixed() +
  labs(x = "Score on Q1", y = "Score on Q2")) %>%
  ggsave(filename = "../figs/fig3.png", dpi = 600, width = 6, height = 4, units = "in")


(ggplot(week7_tbl, aes(x = gender, y = timeSpent)) +
  geom_boxplot() + 
  labs(x = "Gender", y = "Time Elapsed (mins)")) %>%
  ggsave(filename = "../figs/fig4.png", dpi = 600, width = 6, height = 4, units = "in")
...
(ggplot(week7_tbl, aes(x = q5, y = q7, color = condition)) + 
  ...
  ggsave(filename = "../figs/fig5.png", dpi = 600, width = 6, height = 4, units = "in")